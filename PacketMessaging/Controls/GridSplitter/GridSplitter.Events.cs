    ...
    public partial class GridSplitter
    {
		...
		private bool HorizontalMove(double horizontalChange)
        {
            ...
            if (!IsStarColumn(CurrentColumn))
            {
                ...
                if (!SetColumnWidth(CurrentColumn, horizontalChange, GridUnitType.Pixel, _gridSplitterTargetedColumnIndex))
                {
					return true;
                }
            }
            ...
            {
                if (!SetColumnWidth(SiblingColumn, horizontalChange * -1, GridUnitType.Pixel, _gridSplitterSiblingColumnIndex))
                {
                    return true;
                }
            }
            ...
            else
            {
                ...
                foreach (var columnDefinition in Resizable.ColumnDefinitions)
                {
                    ...
                    {
                        SetColumnWidth(CurrentColumn, horizontalChange, GridUnitType.Star, _gridSplitterTargetedColumnIndex);
                    }
                    else if (columnDefinition == SiblingColumn)
                    {
                        SetColumnWidth(SiblingColumn, horizontalChange * -1, GridUnitType.Star, _gridSplitterSiblingColumnIndex);
                    }

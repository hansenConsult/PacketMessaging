...
using static PacketMessaging.Views.MainPage;

//namespace Microsoft.Toolkit.Uwp.UI.Controls
namespace PacketMessaging.Controls.GridSplitter
{
    /// <summary>
    ...
    public partial class GridSplitter : Control
    {
        ...
        private ColumnDefinition CurrentColumn
        {
            get
            {
                ...
                _gridSplitterTargetedColumnIndex = GetTargetedColumn();

                if ((_gridSplitterTargetedColumnIndex >= 0)
                    && (_gridSplitterTargetedColumnIndex < Resizable.ColumnDefinitions.Count))
                {
                    return Resizable.ColumnDefinitions[_gridSplitterTargetedColumnIndex];
                }
        ...
        private ColumnDefinition SiblingColumn
        {
            get
            {
                ...
                _gridSplitterSiblingColumnIndex = GetSiblingColumn();

                if ((_gridSplitterSiblingColumnIndex >= 0)
                    && (_gridSplitterSiblingColumnIndex < Resizable.ColumnDefinitions.Count))
                {
                    return Resizable.ColumnDefinitions[_gridSplitterSiblingColumnIndex];
                }
